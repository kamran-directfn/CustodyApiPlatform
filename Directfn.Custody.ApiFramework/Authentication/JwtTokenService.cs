...
namespace Directfn.Custody.ApiFramework.Authentication
{
    public sealed class JwtTokenService : IJwtTokenService
    {
        ...
        public TokenResult GenerateAccessToken(JwtTokenRequest request)
        {
            ...
            {
                claims.Add(new Claim("first_name", request.FirstName));
            }

            if (!string.IsNullOrWhiteSpace(request.ImageURL))
            {
                claims.Add(new Claim("image_url", request.ImageURL));
            }
            ...
            if (!string.IsNullOrWhiteSpace(request.PortfolioGroupId))
            {
                claims.Add(new Claim("portfolio_group_id", request.PortfolioGroupId));
            }
