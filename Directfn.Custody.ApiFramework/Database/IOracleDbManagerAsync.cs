...
{
    Task<DataTable> GetQueryResultAsync(
        string query,
        IEnumerable<OracleParameter>? parameters = null,
        CancellationToken cancellationToken = default);

    Task<DataTable> GetStoredProcedureResultAsync(
        string procedureName,
        IEnumerable<OracleParameter>? parameters = null,
        CancellationToken cancellationToken = default);

    Task<DataTable> GetStoredProcedureRefCursorAsync(
        string procedureName,
    ...
    Task<List<T>> GetStoredProcedureRefCursorAsync<T>(
    string procedureName,
    IEnumerable<OracleParameter> parameters,
    string refCursorParameterName,
    CancellationToken cancellationToken = default)
